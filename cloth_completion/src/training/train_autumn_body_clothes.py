...
from src.dataset import FastDataLoader as DataLoader
...
from src.pyutils import logging
logger = logging.get_logger()

from src.pyutils.training import (
    validate,
    _process_losses,
    to_device,
...
def train(
    ...
    for batch in train_loader:
        ...
        if iteration and iteration % config.save_every == 0:
            ...
            #     config.output_dir, "models", f"{iteration:06d}"
            # )
            # os.makedirs(out_models_dir, exist_ok=True)
            # logger.info(f"saving model to {out_models_dir}")
            # # TODO: deprecate individual model saving
...
def main(config):
    ...
    if "val" in config.split and local_rank == 0:
        val_data = DatasetClass(
            **config.split.val, **config.dataset, cameras=train_data.cameras
        )
    ...
    if "test" in config.split and local_rank == 0:
        test_data = DatasetClass(
            **config.split.test, **config.dataset, cameras=train_data.cameras
        )
