#TODO: This file is used to train the keypoint color&location detector in color_corners_and_centers_detector.py
import os.path
...
class PatchKeypointDataset(Dataset):
    def __init__(self, path_dataset_file, debug_dir, num_random_keypoints):
        ...
        self.num_background_points_if_empty = 100
        self.std_centers = 0
        self.patch_size = 64
    ...
    def getitem(self, item):
        ...
        # consider the case the patch has zero annotations - put some background samples
        if (item["centers"].shape[0] + item["corners"].shape[0]) == 0:
            rand_locations = np.random.choice(item["patch"].shape[0], (self.num_background_points_if_empty, 2))
            keypoints = np.pad(rand_locations, ((0, 0), (0, 2)))
        else:
            keypoints = np.concatenate((np.pad(item["corners"], ((0, 0), (0, 1))), item["centers"]), axis=0)
            type_label = (1 + 1 * (keypoints[:, 2:3] != 0)).astype(int) # corner=1, center = 2,
        ...
        # if after the augmentations no annotation survives
        if np.any(keypoints_inside_idx) == False:
            return self.getitem(np.random.choice(len(self.items)))

        background = keypoints[keypoints[:, 3] == 0, :]
        corners = keypoints[keypoints[:, 3] == 1, :]
        centers = keypoints[keypoints[:, 3] == 2, :]
        keypoint_segmentation = np.zeros(patch_aug[0].shape[:-1])
        keypoint_segmentation[corners[:, 0], corners[:, 1]] = 1
        keypoint_segmentation[centers[:, 0], centers[:, 1]] = 2
        ...
        out["patch"] = patch_aug[0][:, :, :3].transpose(2, 0, 1).astype(np.float32).copy()
        out["keypoint_segmentation"] = keypoint_segmentation.astype(np.int64).copy()
        ...
        # zero the keypoint entries
        debug_image[out["keypoints"][:, 1], out["keypoints"][:, 0], :] = 0
        debug_image[corners[:, 1], corners[:, 0], 2] = 255  # Red
        debug_image[centers[:, 1], centers[:, 0], 1] = 255  # Green
        debug_image[background[:, 1], background[:, 0], 0] = 255  # Blue

        cv2.imwrite(os.path.join(self.debug_path, out["filename"]), debug_image)
        cv2.imwrite(os.path.join(self.debug_path, 'segmentation_' + out["filename"]), 80 * out["keypoint_segmentation"].astype(np.uint8))
#
        return out
...
class TestDataset(Dataset):
    def __init__(self, path, cameras, frames, bad_files_path):
