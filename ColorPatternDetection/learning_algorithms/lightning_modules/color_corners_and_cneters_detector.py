...
from learning_algorithms.unet.unet_model import UNet
...
class CornersCentersNet(torch.nn.Module):
    def __init__(self, num_classes):
        ...
        self.net = UNet(n_channels=3, n_classes=3)
...
class ColorClassifier(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(saved_image_color.shape[0]):
            cv2.imwrite(os.path.join(save_dir, f'{batch["cam"][i]}_{batch["frame"][i]}_color.png'), np.flip(saved_image_color[i], axis=-1).astype(np.uint8)) # opencv assumes BGR convention
            cv2.imwrite(os.path.join(save_dir, f'{batch["cam"][i]}_{batch["frame"][i]}_type.png'), np.flip(saved_image_type[i], axis=-1).astype(np.uint8)) # opencv assumes BGR convention
    ...
    def training_step(self, batch, batch_idx):
        ...
        labels_prediction_type_samples = labels_prediction_type[sample_idx[:, 0], :, sample_idx[:, 2], sample_idx[:, 1]] # the locations coordintes should be flipped!

        # random samples determined as background type
        # n_random_samples = 200
        # random_locations = torch.randint(high=image.shape[2], size=(image.shape[0], n_random_samples, 2)).to(device=image.device) # shape: B x #random_samples x 2
        # batch_idx_random_samples = torch.arange(random_locations.shape[0])[:, None, None].expand((random_locations.shape[0:2] + (1,))).to(device=image.device)  # B x #random_samples x 1
        # random_sample_idx = torch.cat((batch_idx_random_samples, random_locations), dim=2).view(-1, 3).long() # (B * #random_samples) x 3
        # labels_prediction_type_random_samples = labels_prediction_type[random_sample_idx[:, 0], :, random_sample_idx[:, 2], random_sample_idx[:, 1]]
        # background_type_labels = torch.zeros(image.shape[0], n_random_samples).long().to(device=image.device)
        # type_labels = torch.cat((type_labels, background_type_labels), axis=1)
        # print(f"Num background: {torch.sum(type_labels == 0)}")
        # print(f"Num corners & centers: {torch.sum(type_labels == 1) + torch.sum(type_labels == 2)}")

        type_labels_one_hot = F.one_hot(type_labels, num_classes=self.num_type_classes)
        # labels_prediction_type_samples = torch.cat((labels_prediction_type_samples, labels_prediction_type_random_samples), axis=0)

        loss_type = F.cross_entropy(labels_prediction_type_samples,
                         type_labels_one_hot.view(-1, self.num_type_classes).to(dtype=torch.float),
                         weight=torch.Tensor([1, 1, 2]).to(device=labels_prediction_type_samples.device)) # inputs & target shape: (B * #samples) x 7

        # concurrence loss for colors
        x = labels_prediction_type.permute(0, 2, 3, 1).reshape(-1, 3)
        x = x.to(dtype=torch.double)
        y = x[:, 1:]  # only corners & centers - not background
        concurrence_keypoints = torch.tril(torch.matmul(y.transpose(1, 0), y) / y.shape[0], diagonal=-1).sum()
        concurrence_background = torch.tril(torch.matmul(x.transpose(1, 0), x) / x.shape[0], diagonal=-1).sum()

        loss = loss_color + loss_type + 10000 * concurrence_keypoints
        self.log("loss", loss)
        return loss
...
if __name__ == '__main__':
    ...
    save_path = '/mnt/home/oshrihalimi/color_pattern_detection/elements_detector_color_U-net_locations_U-net-5_random_samples_0'
    ...
    test_images_path = '/mnt/home/oshrihalimi/Data/ColorPattern/'
